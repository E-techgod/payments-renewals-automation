...
from app.birthday_rules import (
    FixedClock,
    SystemClock,
    build_clock,
    is_birthday_today,
...
def test_parse_birthday_supports_date_passthrough() -> None:
    birthday = date(2000, 1, 1)

    assert parse_birthday(birthday) == birthday


def test_parse_birthday_supports_datetime_passthrough() -> None:
    birthday = datetime(2000, 1, 1, 9, 30, 0, tzinfo=UTC)

    assert parse_birthday(birthday) == date(2000, 1, 1)


def test_parse_birthday_supports_excel_serial() -> None:
    assert parse_birthday(36526) == date(2000, 1, 1)


@pytest.mark.parametrize("raw_value", [float("nan"), float("inf"), float("-inf")])
def test_parse_birthday_returns_none_for_non_finite_excel_serials(
    raw_value: float,
) -> None:
    assert parse_birthday(raw_value) is None
...
def test_parse_birthday_supports_string_formats(raw_value: str, expected: date) -> None:
    assert parse_birthday(raw_value) == expected


def test_parse_birthday_returns_none_for_malformed_string() -> None:
    assert parse_birthday("definitely-not-a-date") is None


@pytest.mark.parametrize("raw_value", ["1/2/03", "13/45/2000", "February 30, 2000"])
def test_parse_birthday_returns_none_for_invalid_date_strings(raw_value: str) -> None:
    assert parse_birthday(raw_value) is None


def test_parse_birthday_long_month_parser_is_locale_independent() -> None:
    ...
    try:
        assert parse_birthday("May 27, 2003") == date(2003, 5, 27)
    finally:
        locale.setlocale(locale.LC_TIME, original_locale)


def test_parse_birthday_returns_none_for_missing_value() -> None:
    assert parse_birthday(None) is None


def test_is_birthday_today_matches_feb_29_on_leap_day() -> None:
    assert is_birthday_today(date(2000, 2, 29), date(2024, 2, 29))


def test_is_birthday_today_matches_feb_29_on_feb_28_in_non_leap_year() -> None:
    assert is_birthday_today(date(2000, 2, 29), date(2023, 2, 28))


def test_is_birthday_today_does_not_match_feb_29_on_feb_28_in_leap_year() -> None:
    assert not is_birthday_today(date(2000, 2, 29), date(2024, 2, 28))


def test_is_birthday_today_matches_only_exact_date_for_normal_birthdays() -> None:
    birthday = date(2000, 1, 2)

    assert is_birthday_today(birthday, date(2026, 1, 2))
    assert not is_birthday_today(birthday, date(2026, 1, 1))
    assert not is_birthday_today(birthday, date(2026, 1, 3))
...
def test_domain_models_store_expected_fields() -> None:
    ...
    birthday_match = BirthdayMatch(client=client, celebrated_year=2026)
    ...
    assert birthday_match.client is client
    assert birthday_match.celebrated_year == 2026
